...
<template>
  <div>
    <h1 class="text-2xl font-bold text-ink-100 mb-6">KZTimer Ranks</h1>
      ...
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-ink-800">
          ...
          <tr v-else-if="!ranks?.length">
            <td colspan="7" class="px-4 py-6 text-center text-ink-500">No ranked players yet.</td>
          </tr>
          <tr v-for="p in ranks" :key="p.steamId" class="hover:bg-ink-900/50">
            ...
            <td class="px-4 py-2 text-right font-mono tabular-nums text-ink-400">{{ p.finishedMaps }}</td>
            <td class="px-4 py-2 text-right font-mono tabular-nums text-ink-400">{{ p.finishedMapsPro }}</td>
            <td class="px-4 py-2 text-right font-mono tabular-nums text-ink-400">{{ p.finishedMapsTp }}</td>
            <td class="px-4 py-2 text-right text-ink-500 text-xs">{{ p.lastSeen }}</td>
          </tr>
        </tbody>
      </table>
    </div>
