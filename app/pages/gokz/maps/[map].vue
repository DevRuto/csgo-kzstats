...
<template>
  <div>
    <div class="mb-6">
      <NuxtLink to="/gokz/maps" class="text-sm text-ink-500 hover:text-ink-300">&larr; All maps</NuxtLink>
      ...
      >
        ...
        <div class="absolute inset-x-0 bottom-0 px-5 py-4">
          ...
          <div class="text-sm text-ink-300 mt-1 flex items-center gap-2">
            <span v-if="map.inRankedPool" class="text-accent">Ranked</span>
      ...
      <template v-else>
        <h1 class="text-2xl font-bold text-ink-100 mt-1">{{ mapName }}</h1>
        <div v-if="map" class="text-sm text-ink-500 mt-1 flex items-center gap-2">
      ...
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-ink-800">
          ...
          <tr v-else-if="!records?.length">
            <td colspan="5" class="px-4 py-6 text-center text-ink-500">No records for this mode/course yet.</td>
          </tr>
          <tr v-for="r in records" :key="r.steamId32" class="hover:bg-ink-900/50">
            <td class="px-4 py-2"><RankBadge :rank="r.rank" /></td>
            <td class="px-4 py-2 text-ink-200">
              <NuxtLink :to="`/players/${r.steamId32}`" class="hover:text-accent">{{ r.alias ?? 'Unknown' }}</NuxtLink>
              <span v-if="r.country" class="text-ink-500 text-xs ml-1">{{ r.country }}</span>
            </td>
            <td class="px-4 py-2 text-right font-mono tabular-nums text-ink-200">{{ formatDurationMs(r.runTimeMs) }}</td>
            <td v-if="type === 'tp'" class="px-4 py-2 text-right font-mono tabular-nums text-ink-400">{{ r.teleports }}</td>
